...
- **PR**:
  - 본문은 `.github/PULL_REQUEST_TEMPLATE.md` 양식(요약 / 검증 / 관련 이슈)을 채운다 — GitHub이 작성창에 자동으로 불러온다.
  - 요약에 변경을 무엇을·왜, 검증에 돌려본 명령·결과를 적는다.
  - 관련 이슈에 반드시 **`Closes #<이슈번호>`** — merge 시 이슈 자동 종료.
  - 영역 라벨은 labeler가 자동 부착, 리뷰어는 CODEOWNERS가 자동 지정.
- **이슈 본문**도 `.github/ISSUE_TEMPLATE/`의 양식(버그 `bug.md` / 작업·기능 `work.md`)을 채운다. AI로 등록할 때도 이 구조를 지킨다.
