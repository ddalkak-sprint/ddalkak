...
bridge node와 DOM의 매칭은 text 내용, bbox IoU, center distance, element kind만으로 수행한다.
`data-dk` 앵커에는 의존하지 않는다 — 앵커는 생성 LLM이 부착하므로 코드에 오류가 있을수록 함께 무너지고
정합성을 독립 검증할 수 없기 때문이다. 요소 단위 정밀 판정은 관찰 기반 검증기(verify-observe)가 전담한다.
`matches[].strategy`는 어떤 신호로 매칭됐는지 기록한다(`text-exact`, `text-partial`, `geometry`, `kind`의 조합).
