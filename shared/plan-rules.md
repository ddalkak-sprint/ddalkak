...
- **비토큰 raw 값도 표에서 최종 코드 표현으로 확정한다 (code 암산 제거·재실행 결정론).**
  화면에 쓰인 raw 시각값(radius/gap/padding/폭 등)의 **distinct 집합**을 표에 모아, 각 값을 미리 환산해 둔다
...
- **warning(완결성)**: 브릿지가 참조한 토큰·raw 시각값(radius/gap/padding/고정 폭·높이)이 토큰 매핑 표에
  없음, 컴포넌트 미언급, 페이지 등록 행 부재. 이 경고가 곧 §4가 막으려는 "code 즉석 환산" 지점이므로,
  지적된 값을 표에 채워 없애는 것을 기본으로 한다.
