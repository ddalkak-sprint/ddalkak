# Figma 추출 규칙 (bridge 단계 SSOT) — v2.1

## 캡처 완결성 게이트

섹션 응답이 `codeSummary`이면 그 응답은 상세 입력으로 간주하지 않는다. `get_metadata`에서 발견한
모든 `metadataLeaf`의 node ID별 상세 `get_design_context` 응답이 캐시에 있어야 `full` 캡처다.
`scripts/mcp-cache.mjs check <cacheDir>`가 실패하면 bridge 생성을 중단하고 누락 ID를 live 추출
대기열로 반환한다. 이 게이트는 스크린샷 기반 vision 보정으로 우회할 수 없다.
...
## 5. 적응형 디자인 — 플랫폼 중립 화면 조건 + 노드 대응 + constraints (3층)
bridge는 CSS breakpoint가 아니라 웹·React Native·iOS·Android가 함께 읽을 수 있는 **관측 사실과 레이아웃 의도**를 기록한다.
`mobile|tablet|desktop`은 하위 호환 이름표일 뿐이며, 특정 플랫폼 문법이나 breakpoint 숫자를 확정하지 않는다.
...
3. **노드 대응과 동작**: 변형 사이에서 동일함이 확실한 노드에만 `matchKey`를 붙인다. 이름만 비슷한 노드를
   억지로 연결하지 않는다. 각 노드의 `sizing`, `behavior`, `constraints`를 합쳐 크기·흐름·겹침·가시성 변화를 표현한다.

plan/code는 이 중립 계약을 대상 플랫폼의 media/container query, window size class, Auto Layout 등으로 변환한다.
...
8. 병합 후 §2 가지치기, §5 적응형 그룹핑 적용 → §8 교차검증 → 최종 JSON 조립.

## 7. 오토레이아웃 캡처
컨테이너 노드(frame/group)의 `layout` 객체를 다음까지 채운다 (퍼블 재현 핵심):
- `mode`(row/column/grid/none), `wrap`(no-wrap/wrap), `padding`([top,right,bottom,left]),
  `gap`(숫자 또는 `@spacing.*`), `primaryAxisAlign`, `counterAxisAlign`.
- 노드 자체 크기 정책은 `sizing.horizontal/vertical.mode`(fixed/content/fill/fraction)에 기록하고,
  확인 가능한 경우 min/max/aspectRatio/grow/shrink를 보존한다.
- `behavior.positioning`은 flow/overlay, overflow는 visible/clip/scroll/auto로 기록한다.
- text 노드의 nowrap/ellipsis/line clamp는 `textBehavior`, image의 cover/contain은 `assetFit`에 기록한다.
- 오토레이아웃이 아닌 절대배치 컨테이너는 `mode: "none"` + 자식 `bbox`/`constraints`로 표현.
- Figma 코드의 `grid`를 flex로 바꾸지 않는다. 겹침 grid도 의미 있는 배치이므로 `layout.mode: "grid"`와
  확인 가능한 track 정보를 보존한다.
...
- 스켈레톤에 없는 노드(재조합 서브트리 내부, vision backfill)만 LLM이 수치를 채우며, 그 서브트리는
  ...
  재조합 후보 목록이다.
- **`detail` 패스의 코드 응답도 기계가 컴파일한다** — `scripts/design-context-to-bridge.mjs`가
  `get_design_context` 코드를 브릿지 노드 초안(구조·텍스트·fills·strokes·radius·effects·layout·
  에셋 ref·앵커 원형 §8-3)으로 결정론 변환한다. 헬퍼 컴포넌트 함수는 호출부 className·props로
  인라인하고, 조건식 텍스트/클래스(`{badge === "coworker" ? …}`, `${isOther ? …}`)는 호출부 props와
  파생 바인딩(`const isOther = badge === "other"`)으로 확정하며, **못 푸는 표현식은 버리지 않고
  `contentExpr`로 원식을 보존**한다(조용한 소실 금지). extractor는 이 초안을 스켈레톤과 병합해
  type(instance/component) 판별만 마무리한다 — 구조·수치·텍스트를 LLM이 재전사하지 않는다.
  실측(untitled-1-858): 수작업 병합에서 났던 결함 4건(아바타 스택 누락·카드별 아바타 ref 오지정·
  앵커 소실·gap 오류)을 컴파일러는 전부 처음부터 정답으로 냈고, 204노드 변환에 0.24초.
...
### 8-3. 앵커 원형 보존 — px 환산은 "이 프레임에서의 값"일 뿐이다
`get_design_context` 코드의 위치 표현에는 **반응형 의도가 실려 있다**: `left-[calc(66.67%-104px)]`
(부모 폭 비례), `right-[Npx]`(반대변 고정), `left-1/2 + -translate-x-1/2`(중앙 고정). 이걸 캡처 프레임
크기 기준 px로 환산해 bbox에만 넣고 원형을 버리면, **부모가 리사이즈될 때의 동작(§5 constraints)이
브릿지에서 통째로 증발한다** — MCP가 준 정보를 추출기가 임의로 판단해 버리는 것이므로 무손실 원칙(§0)
위반이다. 실측: untitled-1-858 카드 그리드의 퍼센트 앵커를 px로만 저장해 반응형 정보가 소실됐었다.
- `scripts/design-context-parse.mjs`가 px bbox와 **함께** 앵커 원형을 내보낸다:
  `constraints.anchorX/anchorY = { kind: percent|opposite|stretch, percent, offset, start, end, translate }` +
  §5 열거값(`horizontal/vertical: center|scale|right|bottom`) 자동 매핑. plain `left/top-[Npx]`만
  생략(그건 bbox가 이미 전부).
- `translate` 플래그는 클래스가 있으면 **무조건 기록**한다 — 요소 크기(hug)를 몰라 수치 보정을
  못 하는 경우에도 원형은 남겨야 center 판정이 산다.
- extractor는 이 constraints를 스켈레톤 병합 시 노드에 그대로 싣고, plan/code는 anchorX/anchorY가
  있는 노드를 px 고정이 아니라 `calc(P%±Npx)`/중앙정렬로 구현해 반응형을 재현한다.
...
- **입력 불변 스킵(시간 절약)**: 캐시 지문은 `meta.sourceFingerprint`, 스키마·규칙·변환기 지문은
  `meta.extractorFingerprint`에 기록한다. 둘 다 기존 bridge와 같을 때만 재사용한다. 소스가 같아도
  추출기 지문이 다르면 새 필드와 버그 수정을 반영하기 위해 캐시에서 다시 생성한다.
- cache final 생성은 `scripts/bridge-from-cache.mjs`를 사용한다. 노드 컴파일, 캐시 URL 자산의 프로젝트
  로컬 적재, screenshot 복사, compact 저장, JSON Schema·스크린샷 검증을 한 번에 수행한다.
...
## 15. 완전성·플랫폼 중립 품질 게이트
- 완전 추출은 `meta.completeness: "full"`, 패스 실패가 하나라도 있으면 `"partial"`과 `meta.errors[]`를 기록한다.
- 모든 산출 필드는 `bridge.schema.json`에 정의돼야 하며 저장 전 JSON Schema 검증을 통과해야 한다.
- adaptive group은 변형 이름이 중복되면 안 되고, 추론 조건에는 confidence가 있어야 한다.
- 변형 간 동일 노드는 `matchKey`로 연결한다. 근거가 없으면 생략하고 검증 결과를 조건부 통과로 남긴다.
- bridge에는 CSS/Tailwind/SwiftUI/Compose 문법을 넣지 않는다. 구현 문법은 plan/code가 대상 플랫폼에 맞게 선택한다.
- 경고가 남은 브릿지는 실패시키지 않되 `조건부 통과`로 보고해 하류가 불확실성을 알 수 있게 한다.
