...
7. DOM snapshot을 수집하고 text/bbox/kind로 bridge node와 매칭해 advisory computed style checks를 만든다.
   DOM이 없는 런타임(캔버스 렌더)에서는 이 단계를 생략하고 픽셀 판정만 남긴다.
...
`bridge.verify.targets[]`가 있으면 `--target <id>`로 검증 runtime을 선택한다. `web`과 `flutter` target은 Playwright provider로 캡처한다 — Flutter Web은 자체 런타임(예: `fvm flutter run -d web-server`)을 verify 밖에서 띄운 뒤 그 URL을 넘긴다. Flutter는 canvas로 렌더돼 의미 있는 DOM이 없으므로 DOM/style check는 생략하고 픽셀 diff로만 판정하며 style 판정은 `n/a`로 기록한다. Android/iOS/React Native target은 provider가 아직 없어 blocked execution error로 중단된다. 앱 target은 code 단계에서 deep link나 preview route 같은 검수용 진입점을 함께 만들어야 한다.
...
`visual.json`에는 v2부터 `version: "2.0"`, `statuses`, `matches`, `checks`가 포함된다. DOM이 없는 타깃에서는 `statuses.style`이 `"n/a"`이고 `matches`·`checks.items`는 비어 있다.
