...
7. DOM snapshot을 수집하고 `data-dk` exact match를 우선 사용해 bridge node와 deterministic하게 매칭한다.
   `data-dk`가 없으면 text/bbox/kind fallback으로 advisory computed style checks를 만든다.
8. duplicate/unknown/missing `data-dk` anchor를 advisory diagnostics로 기록한다.
9. `visual.json`과 사람용 `verify.md`를 `.ddalkak/reports/`에 기록한다.
...
`visual.json`에는 v2부터 `version: "2.0"`, `statuses`, `anchors`, `matches`, `checks`가 포함된다.
