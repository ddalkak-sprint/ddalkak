---
name: verify
description: 생성된 코드가 Figma 디자인과 같은지 LLM 없이 정적 visual verification engine으로 검증한다. Playwright 캡처, 기준 screenshot 픽셀 diff, bridge bbox 영역별 mismatch를 계산해 리포트한다. 딸깍 파이프라인 4단계, 또는 사용자가 구현이 디자인과 맞는지 확인할 때 사용.
---
...
7. `visual.json`과 사람용 `verify.md`를 `.ddalkak/reports/`에 기록한다.
...
- pixel diff는 root cause 확정기가 아니라 mismatch signal generator다.
