...
| | |
|---|---|
| 만들 것 | `skills/verify/SKILL.md`, `skills/finalize/SKILL.md` 구현 (+ `scripts/visual-verify/`, `shared/visual-verify.md`) |
...
모든 작업은 **이슈 단위**로 돌린다: 이슈 등록(`/ddalkak:issue`) → 배정 → 구현·PR(`/ddalkak:hunt`) → review → merge.
각자 맡은 이슈는 각자의 AI로 해결하고, **내 이슈가 아닌 버그는 이슈 등록만** 한다.
컨벤션 전체(제목·라벨·배정·브랜치·PR 규칙): [AGENT.md](AGENT.md)
